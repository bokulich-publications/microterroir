...
setwd('/Users/lfloerl/Desktop/MICROTERROIR/Data/DifferentialAbundance/Pathogens')
...
physeq_genus <- tax_glom(physeq_GVpathogens, taxrank = "Species")  # Aggregate at Genus level
...
prev_df <- data.frame(
  Genus = tax_table(physeq_genus)[, "Species"],
  Prevalence = rowSums(otu_binary) / ncol(otu_binary))
# Save to CSV
write.csv(prev_df, "FunGrapevinePathogens_Species_Prevalence.csv", row.names = FALSE)
...
physeq_GVpathogens_clean <- subset_samples(physeq_GVpathogens, complete.cases(sample_data(physeq)))
dim(physeq_GVpathogens@otu_table)
# 340  32
dim(physeq_GVpathogens@otu_table)
# 340  34

output <- ancombc2(
  data = physeq_GVpathogens_clean, 
  tax_level = 'Genus',
  ...
  pseudo_sens = TRUE,  
  p_adj_method = "holm", 
  prv_cut = 0, # note, setting this to 0 keeps all yeasts but then we cannot control for year anymore  
  s0_perc = 0.05,
  struc_zero = FALSE, 
  neg_lb = FALSE,
  alpha = 0.05, 
