...
otu_data_raw <- read.csv("/Users/lfloerl/Desktop/MICROTERROIR/Data/DifferentialAbundance/berries_harvest_cOTUs_rarefied_labled.tsv", sep = '\t', row.names = 1, check.names = FALSE,encoding = "UTF-8")
...
# Now do the same for median_temperature
# Determine the optimal number of clusters for median_temperature using the elbow method
...
# Apply k-means clustering for median_temperature with optimal k (set manually after checking the plot)
optimal_k_temp <- 2  # Change this based on the elbow method result
kmeans_result_temp <- kmeans(hmp_samples_filtered$median_temperature, centers = optimal_k_temp)
# Assign cluster labels for median_temperature
hmp_samples_filtered <- hmp_samples_filtered %>%
  mutate(temp_category = ifelse(kmeans_result_temp$cluster == 1, "low", "high"))
# Convert to factor for plotting
hmp_samples_filtered$temp_category <- factor(hmp_samples_filtered$temp_category, levels = c("low", "high"))
...
ggplot(hmp_samples_filtered, aes(x = rownames(hmp_samples_filtered), y = median_temperature, 
  ...
  geom_point(size = 3) +
  scale_color_manual(values = c("low" = "blue", "high" = "red")) +
  labs(x = "Sample ID", y = "Median Temperature", color = "Category", shape = "Year") +
  theme_minimal() +
  theme(axis.text.x = element_blank(), axis.ticks.x = element_blank())
...
# Now do the same for median_temperature
# Determine the optimal number of clusters for median_temperature using the elbow method
Altitude_df <- as.data.frame(hmp_samples_filtered$Altitude)
colnames(Altitude_df) <- "Altitude"
unique_vals <- length(unique(hmp_samples_filtered$Altitude))
fviz_nbclust(Altitude_df, kmeans, method = "wss", k.max = min(10, unique_vals - 1))
# Apply k-means clustering for median_temperature with optimal k (set manually after checking the plot)
optimal_k_temp <- 2  # Change this based on the elbow method result
kmeans_result_altitude <- kmeans(hmp_samples_filtered$Altitude, centers = optimal_k_temp)
# Assign cluster labels for median_temperature
hmp_samples_filtered <- hmp_samples_filtered %>%
  mutate(altitude_category = ifelse(kmeans_result_altitude$cluster == 1, "low", "high"))
# Convert to factor for plotting
hmp_samples_filtered$altitude_category <- factor(hmp_samples_filtered$altitude_category, levels = c("low", "high"))
# Scatterplot for median_temperature with labeled clusters
ggplot(hmp_samples_filtered, aes(x = rownames(hmp_samples_filtered), y = Altitude, 
                                 color = altitude_category, shape = as.factor(Year))) +
  geom_point(size = 3) +
  scale_color_manual(values = c("low" = "blue", "high" = "red")) +
  labs(x = "Sample ID", y = "Altitude", color = "Category", shape = "Year") +
  theme_minimal() +
  theme(axis.text.x = element_blank(), axis.ticks.x = element_blank())
...
hmp_samples_filtered <- hmp_samples_filtered %>%
  ...
  select(SampleID, Name, Year, Plot, Altitude, Average_slope, Exposition, Average_radiation, average_rh, median_rh, median_temperature, id, temp_category, rh_category, altitude_category)
...
# Apply Wilcoxon test correction
obj <- mutate_obs(obj, "diff_table",
                  wilcox_p_value = p.adjust(wilcox_p_value, method = "fdr"))

# ONLY show significantly enriched/depleted features
...
obj %>%
  ...
  heat_tree(node_label = cleaned_names,
            ...
            node_color_range = c("#39558CFF", "gray", "#B8DE29FF"), # Blue for depletion, Green for enrichment
            node_size_axis_label = "OTU count",
            node_color_axis_label = "log2(median counts)",
            layout = "da", initial_layout = "re",
            #title = "high vs low RH",
            output_file = "/Users/lfloerl/Desktop/MICROTERROIR/Figures/RH_differential_heat_tree.png")

set.seed(1)
obj %>%
  mutate_obs("cleaned_names", gsub(taxon_names, pattern = "\\[|\\]", replacement = "")) %>%
  metacoder::filter_taxa(grepl(cleaned_names, pattern = "^[a-zA-Z]+$")) %>%
  metacoder::filter_taxa(taxon_ranks == "f", supertaxa = TRUE) %>%
  heat_tree(node_label = cleaned_names,
            node_size = n_obs, # number of OTUs
            node_color = log2_median_ratio, # difference between groups
            node_color_interval = c(-10, 10), # symmetric interval
            ...
            layout = "da", initial_layout = "re",
            #title = "high vs low RH",
            output_file = "/Users/lfloerl/Desktop/MICROTERROIR/Figures/RH_differential_heat_tree_F.png")
...
obj %>%
  mutate_obs("cleaned_names", gsub(taxon_names, pattern = "\\[|\\]", replacement = "")) %>%
  metacoder::filter_taxa(grepl(cleaned_names, pattern = "^[a-zA-Z]+$")) %>%
  metacoder::filter_taxa(taxon_ranks == "f", supertaxa = TRUE) %>% # subset to the order rank
  heat_tree(node_label = cleaned_names,
            node_size = n_obs, # number of OTUs
            node_color = log2_median_ratio, # difference between groups
            node_color_interval = c(-10, 10), # symmetric interval
            ...
            layout = "da", initial_layout = "re", 
            output_file = "/Users/lfloerl/Desktop/MICROTERROIR/Figures/Temp_differential_heat_tree.png")
...
obj$data$diff_table <- compare_groups(obj, data = "tax_abund",
                                      cols =  hmp_samples$SampleID,
                                      groups = hmp_samples$altitude_category)
# wilcox test (corrected)
obj <- mutate_obs(obj, "diff_table",
                  wilcox_p_value = p.adjust(wilcox_p_value, method = "fdr"))
# ONLY show sign. enriched features
...
obj %>%
  ...
  heat_tree(node_label = cleaned_names,
            ...
            node_color_axis_label = "Log 2 ratio of median counts",
            layout = "da", initial_layout = "re", 
            output_file = "/Users/lfloerl/Desktop/MICROTERROIR/Figures/Altitude_differential_heat_tree.png")


########################################################################

# Get per-taxon abundance between the years --> this doesn't work yet
obj$data$tax_abund <- calc_taxon_abund(obj, "tax_data",
                                       cols = hmp_samples$SampleID)
obj$data$diff_table <- compare_groups(obj, data = "tax_abund",
                                      cols =  hmp_samples$SampleID,
                                      groups = hmp_samples$Year) 
